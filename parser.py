...
def parse_pdf(file_obj):
    ...
    if abstract_index != -1:
        ...
        extracted_data['Authors'] = "\n".join([l['text'] for l in authors_lines]).strip()
    ...
    standard_headings = [
        ...
        "Background", "Related Work", "Experimental Setup", "Implementation", "Future Work"
    ]
    ...
    while current_index < len(all_lines):
        ...
        is_all_caps = line['text'].isupper() and len(line['text']) > 3
        ...
        for h in standard_headings:
            # Regex: Start of line, optional number (1. or 1 or I.), optional space, heading, optional punctuation (: or .), end of line
            # We use re.IGNORECASE
            # Expanded regex to handle "1. Introduction" or "I. Introduction" or "Introduction."
            pattern = rf"^(?:(?:\d+\.|[IVX]+\.)\s*)?{re.escape(h)}[:.]?$"
        ...
        if not is_heading:
            # Conditions for dynamic heading:
            # 1. Bold AND (size < title) AND (short)
            # 2. All Caps AND (size < title) AND (short)
            # 3. Numbered start (e.g. "2.1 Analysis") AND (Bold OR All Caps OR Larger than text?) -> Let's stick to user's "Bold and dark" or "Bigger"
            
            is_short = len(text) < 100
            is_smaller_than_title = size < max_size
            
            # Check for "Bold and dark" (we use is_bold)
            if is_bold and is_smaller_than_title and is_short:
                is_heading = True
                heading_name = text
            
            # Check for "All Caps" (common for headings even if not bold)
            elif is_all_caps and is_smaller_than_title and is_short:
                is_heading = True
                heading_name = text
        ...
        if current_section:
            ...
            if current_section_font_size is None:
                ...
                # Check if size is "same" (allow small tolerance, e.g., 1.0 pt)
                # User said "same size", but PDF fonts can vary slightly (e.g. 11.98 vs 12.0)
